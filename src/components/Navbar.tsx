        ...
        <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
          ...
          {isOnline ? (
            pendingOfflineCount > 0 ? (
              ...
              >
                ...
                <span className="hidden sm:inline text-[11px] font-semibold">
                  {isSyncing ? 'Menyinkronkan...' : `${pendingOfflineCount} Tersimpan (Sync)`}
                </span>
                <span className="sm:hidden text-[10px] font-bold">
                  {isSyncing ? 'Sync' : `${pendingOfflineCount} Sync`}
                </span>
              </button>
            ) : (
              <div
            ...
            <div
              ...
              className="h-8 sm:h-9 px-1.5 sm:px-2 rounded-lg bg-rose-500/20 border border-rose-500/40 text-rose-300 text-xs font-medium cursor-default select-none animate-pulse flex items-center justify-center gap-1 shrink-0"
              title="Koneksi Terputus - Mode Offline Aktif. Data presensi Anda aman tersimpan sementara di localStorage."
            >
              ...
              {pendingOfflineCount > 0 && (
                <span
                  className="px-1 py-0.2 rounded-full bg-rose-600 text-white text-[9px] font-bold shrink-0"
                  title={`${pendingOfflineCount} presensi tersimpan di localStorage`}
                >
                  {pendingOfflineCount}
                </span>
              )}
